...
import { LogOut, Settings, User, LayoutDashboard, Users, Briefcase, FileText, ChevronUp, ChevronDown, Menu } from 'lucide-react';
   ...
   useEffect(() => {
      const fetchUserData = async () => {
         try {
            ...
            if (response.data && response.data.adminInfo) {
               setUserInfo({
                  email: response.data.adminInfo.email || '',
                  accessLevel: response.data.adminInfo.accessLevel || 'N/A'
               });
               setAuthError(null);
            } else {
            ...
            if (error.response?.status === 401) {
               ...
               setAuthError('Profile not found. Please check your admin setup.');
            } else {
         ...
         <div className={`flex flex-col h-screen bg-gray-100 border-r border-gray-300 w-64 fixed transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
            ...
            <div className="p-4 border-b border-gray-300">
               <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-gray-800">EMPOWER PWD</span>
               </div>
            </div>
